    ...
    <div class="pj-import-input">
      <header class="pj-dialog-header">
        ...
        <p class="pj-dialog-subtitle">
          Three ways in, all feed the same review flow.
        </p>
      </header>

      <div class="pj-import-input-body">
        <div
          class="pj-drop-zone"
          onDragOver={(e) => e.preventDefault()}
          onDrop={handleDrop}
        >
          <span class="pj-drop-zone-headline">
            <span aria-hidden="true">⬇</span> Drop a <code>.freshet.json</code> file here
          </span>
          <span class="pj-drop-zone-or">— or —</span>
          <button
            type="button"
            class="pj-btn"
        ...
        <label class="pj-import-paste-label" for="pj-import-paste">
          Or paste bundle JSON directly:
        </label>
        <textarea
          id="pj-import-paste"
          class="pj-import-paste"
        ...
        {errors.length > 0 ? (
          <div class="pj-errors" role="alert">
            <strong>Couldn't parse this bundle:</strong>
            {errors.map((err) => (
              <div key={err}>• {err}</div>
            ))}
          </div>
      ...
      <footer class="pj-dialog-footer">
        <span class="pj-import-input-hint">
          {text.trim()
            ? `${new Blob([text]).size} B pasted`
            : 'No input yet.'}
        </span>
